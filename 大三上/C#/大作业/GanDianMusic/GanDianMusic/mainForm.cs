...
namespace GanDianMusic
{
    public partial class mainForm : Form
    {
        ...
        private string defaultSongsFilePath = @"C:\Users\Music";
        ...
        public mainForm()
        {
            ...
            MenuItem item3 = new MenuItem(Resources.user, "About");
            ...
            cmsSongListMenu.BackColor = Color.DarkSlateGray;
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            ReloadStatus();
            //读取播放器列表历史记录
            localSongsList = ReadHistorySongsList();
            favoriteSongsList = ReadFavorSongsList();
        ...
        {
            //暂停按钮
            ttbbtnPlayPause = new ThumbnailToolbarButton(GanDianMusic.Properties.Resources.播放icon, "播放");
            ttbbtnPlayPause.Enabled = true;
            ttbbtnPlayPause.Click += new EventHandler<ThumbnailButtonClickedEventArgs>(btnPlay_Click);

            //上一首按钮
            ttbbtnPre = new ThumbnailToolbarButton(GanDianMusic.Properties.Resources.上一首icon, "上一首");
            ttbbtnPre.Enabled = true;
            ttbbtnPre.Click += new EventHandler<ThumbnailButtonClickedEventArgs>(btnBack_Click);

            //下一首按钮
            ttbbtnNext = new ThumbnailToolbarButton(GanDianMusic.Properties.Resources.下一首icon, "下一首");
        ...
        {
            //设置专辑封面为默认
            pbAlbumImage.Image = GanDianMusic.Properties.Resources.defaultAlbum;
            lbMusicName.Text = "GanDianMusic";
        ...
        private void StarUp(string flag)
        {
            ...
            if (flag.Equals("1"))
            {
                ...
                Rkey.SetValue(keyName, path + @"\GanDianMusic.exe");
            }
            else
            {
                if (Rkey != null)
         ...
         * 保存播放列表记录到数据库
         */
        private void SaveSongsListHistory(string dbName, List<SongsInfo> songsList)
        {
            if(dbName.Equals("music"))
            {
                MusicDao musicDao = new MusicDao();

                MusicDao.ClearMusic();
                MusicDao.Addmusic(songsList);
            }
            if (dbName.Equals("favor"))
            {
                FavorDao favorDao = new FavorDao();

                FavorDao.ClearFavor();
                FavorDao.Addmusic(songsList);
            }
        }
         ...
         * 读取数据库到列表
         */
        private List<SongsInfo> ReadHistorySongsList()
        {
            List<SongsInfo> resSongList = new List<SongsInfo>();
            MusicDao musicDao = new MusicDao();

            DataSet ds = musicDao.QueryAllMusic();
            for(int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                SongsInfo mus = new SongsInfo((string)ds.Tables[0].Rows[i][1]);
                mus.FileName = (string)ds.Tables[0].Rows[i][0];
                mus.Filesize = (string)ds.Tables[0].Rows[i][2];
                mus.Artist = (string)ds.Tables[0].Rows[i][3];
                mus.Album = (string)ds.Tables[0].Rows[i][4];
                mus.AlbumImage = null;
                mus.Year = (string)ds.Tables[0].Rows[i][6];
                mus.OriginName = (string)ds.Tables[0].Rows[i][7];
                mus.Duration = (string)ds.Tables[0].Rows[i][8];
                mus.ByteRate = (string)ds.Tables[0].Rows[i][9];
                mus.SmallAblum = null;
                resSongList.Add(mus);
            }
        ...
        private List<SongsInfo> ReadFavorSongsList()
        {
            List<SongsInfo> resSongList = new List<SongsInfo>();
            FavorDao favorDao = new FavorDao();

            DataSet ds = favorDao.QueryAll();
            for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
            {
                SongsInfo mus = new SongsInfo((string)ds.Tables[0].Rows[i][1]);
                mus.FileName = (string)ds.Tables[0].Rows[i][0];
                ...
                mus.Artist = (string)ds.Tables[0].Rows[i][3];
                mus.Album = (string)ds.Tables[0].Rows[i][4];
                //mus.AlbumImage = (Image)ds.Tables[0].Rows[i][5];
                mus.AlbumImage = null;
                mus.Year = (string)ds.Tables[0].Rows[i][6];
                mus.OriginName = (string)ds.Tables[0].Rows[i][7];
                mus.Duration = (string)ds.Tables[0].Rows[i][8];
                mus.ByteRate = (string)ds.Tables[0].Rows[i][9];
                //mus.SmallAblum = (Image)ds.Tables[0].Rows[i][10];
                mus.SmallAblum = null;
                resSongList.Add(mus);
            }
        ...
        private void FormControlButton_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult result = MessageBox.Show("是否退出播放器？", "感电音乐", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void lbMenu_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            //绘制底色
            Color backgroundColor = Color.DarkSlateGray;             //背景色
        ...
        private void tsmiFavorite_Click(object sender, EventArgs e)
        {
            ...
            SaveSongsListHistory("favor", favoriteSongsList);
        }
        ...
        private void tsmiRemoveSongFromList_Click(object sender, EventArgs e)
        {
            ...
            if (deleteSongFormList.ShowDialog() == DialogResult.OK)
            {
                ...
                if (lbMenu.SelectedIndex == 0)
                {
                    ...
                    SaveSongsListHistory("music", localSongsList);
                    AddSongsToListView(localSongsList);
                }
        ...
        private void btnPlayMode_Click(object sender, EventArgs e)
        {
            ...
            if (currPlayMode == PlayMode.SingleLoop)
                btnPlayMode.BackgroundImage = GanDianMusic.Properties.Resources.单曲循环;
            else if (currPlayMode == PlayMode.ListLoop)
                btnPlayMode.BackgroundImage = GanDianMusic.Properties.Resources.列表循环;
            else
                btnPlayMode.BackgroundImage = GanDianMusic.Properties.Resources.随机播放;
        }
        #endregion
        ...
        private void lvSongList_SelectedIndexChanged(object sender, EventArgs e)
        {
